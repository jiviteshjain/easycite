...
- **`dblp.org`, `api2.openreview.net`, `arxiv.org` / `export.arxiv.org`, `aclanthology.org`, `api.crossref.org`, `www.ebi.ac.uk` (Europe PMC)** — your search queries are sent to these public paper databases, and BibTeX entries are fetched from them. The query text (what you type, or the text under your cursor used to seed a search) is the only document content that ever leaves Overleaf, and it goes only to whichever of these sources you have enabled. Some sources (currently Crossref) offer a "polite pool" with higher rate limits if you include a contact email; you can optionally provide one in the settings popup, in which case it is sent along with requests to those sources. If you see frequent 429 / rate-limit errors from these sources in the overlay, consider providing your email. It is stored alongside your other settings in `chrome.storage.sync` (synced across your Chrome profile if you have Chrome sync enabled) and otherwise only sent to the polite-pool sources above.
